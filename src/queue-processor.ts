...
async function processMessage(dbMsg: DbMessage): Promise<void> {
    try {
        ...
        if (agent.observer_enabled && typeof invokeResult !== 'string') {
            ...
            const reflectionThreshold = agent.observer_reflection_threshold ?? 40_000;
            const observerSrc = settings?.workspace?.observer_src;
            const observerPython = settings?.workspace?.observer_python;
            runObserver(agentId, invokeResult.messages || [
                { role: 'user', content: message },
                { role: 'assistant', content: response },
            ], workspacePath, 'claude', tokenThreshold, reflectionThreshold, observerSrc, observerPython).catch(err => log('WARN', `Observer failed for ${agentId}: ${(err as Error).message}`));
        }
